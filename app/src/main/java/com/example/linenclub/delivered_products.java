...
public class delivered_products extends AppCompatActivity {
    ListView lv;
    String[] discount,date,amount,stat,mid;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_products);
        lv = (ListView)findViewById(R.id.lst_product);
        SharedPreferences sh = PreferenceManager.getDefaultSharedPreferences(getApplicationContext());
        String hu = sh.getString("ip", "");
        String url = "http://" + hu + ":8000/myapp/android_view_delivered_products/";
...
//                                String lid= jsonObj.getString("lid");
                                ...
                                for (int i = 0; i < js.length(); i++) {
                                    ...
                                    discount[i] = u.getString("discount");
...
//                                listView1.setAdapter(ad);
                                lv.setAdapter(new custome_delivered_product(getApplicationContext(), discount,date,amount,stat,mid));
//
                            }
